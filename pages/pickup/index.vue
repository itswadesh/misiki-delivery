  ...
  <div>
    ...
    >
      <template v-slot="{ result: { error, data }, isLoading }">
        ...
        <div v-else-if="data">
          <div class="fx" v-if="data.delivery && data.delivery.all">
            <h1 style="color: blue;">
              {{ data.delivery.all.total | currency(settings.currency_symbol) }}
            </h1>
            <h1 style="color: red;">
              &nbsp;-
              {{
                data.delivery.cancelled &&
                data.delivery.cancelled.total
            ...
            <h1 style="color: green;">
              &nbsp; =
              {{
                (data.delivery.all.total - data.delivery.cancelled.total)
                  | currency(settings.currency_symbol)
              }}
            </h1>
          </div>
          <div
            ...
            v-for="(c, ix) in data.todaysChefs"
            :key="ix"
          >
            <nuxt-link :to="'/pickup/' + c._id.id">
              ...
              <h1 class="text-3xl font-black text-red-500">
                {{ c.amount | currency(settings.currency_symbol) }} ({{
                  c.count
                }})
              </h1>
            </nuxt-link>
