<template>
  <div>
    ...
    <div>
      <ul class="p-left">
        <li class="card" v-for="(o, ix) in orders" :key="ix">
          ...
          <div v-for="(i, ixx) in o.data" :key="'i-' + ixx" class="fx customer">
            <div style="color: #333;">{{ i.item }}</div>
            <div class="pt-1">{{ i.name }} ({{ i.phone }})</div>
            <div class="pt-1">
              {{ i.qty }} * {{ i.rate | currency(settings.currency_symbol) }} =
          ...
          <div class="center pt-1">
            <Button v-model="o.status" value="Cancelled" class="btn4 font"
              >Cancelled</Button
            >
            <Button v-model="o.status" value="Prepared" class="btn1 font"
              >Prepared</Button
            >
            <Button
              v-model="o.status"
            ...
            >
            <Button v-model="o.status" value="Delivered" class="btn3 font"
              >Delivered</Button
            >
          </div>
        </li>
    ...
    <nuxt-link to="/delivery/" class="history-button"
